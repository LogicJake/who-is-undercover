...
# @Last Modified time: 2019-03-14 20:06:16
from app import db
import time
...
def enter_room(room_id, uid):
    ...
    if has_come is not None:
        ...
        message = wrap_enter_message(
            room_id, room_owner, word, has_come.index, bad_num, num, white_number)
        send_message(uid, message)
        return
    ...
    message = wrap_enter_message(
        room_id, room_owner, word, your_number, bad_num, num, white_number)
    send_message(uid, message)
...
def wrap_enter_message(room_id, room_owner, word, number, bad_num, num, white_number):
    insert_white = '' if white_number == -1 else '，1个白板'
    white_num = 0 if white_number == -1 else 1

    message = '房  号：{}\n房  主：{}\n词  语：{}\n你  是：{}号\n配  置：{}个卧底，{}个平民'\
        .format(room_id, room_owner, word, number, bad_num, num - bad_num - white_num, insert_white)
    return message
